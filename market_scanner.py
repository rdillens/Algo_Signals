...
def main(user=None, market=None, exchange=None, product_type=None, ticker=None):
    if ticker is None: ticker = hf.get_ticker()
    ...
    # candle_1m_df = hf.process_ticker_hist(ticker, interval='1m')
    ...
    #     user = hf.get_username(user)
    #     # Choose what market to scan: Stock, Crypto, or Forex
    #     market = hf.choose_market()
    #     # Choose the exchange for crypto, stocks use 'US', 
    #     exchange = hf.choose_exchange(market)
    #     # Choose type of product
    #     product_type = hf.choose_product_type(market, exchange)
    #     # Create list of products to scan
    #     product_df = hf.gen_product_df(market, exchange, product_type).sort_values().reset_index(drop=True)
    #     # Query with each product in dataframe to get current market info
    #     product_df = hf.get_market_info(product_df, market, exchange, product_type, engine)
